...
import com.bengshiwei.zzj.app.reptile.TeleplayDetailsReptile;
...
public class M3U8MovieReptile implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        Logger logger = Logger.getLogger("M3U8MovieReptile");
...
//        urls .addAll(page.getHtml().xpath("//div[@class='news-img']//a/@href").all());
//        imgs.addAll(page.getHtml().xpath("//div[@class='news-img']//a/img/@src").all());
        M3U8MovieDetailsModel movieDetailsModel = null;
        for (int i = 0; i < titles.size(); i++) {
            List<M3U8MovieDetailsModel> movieDetailsModels = ReptileUtils.getM3U8MovieUrl(urls.get(i));
            if (movieDetailsModels != null && movieDetailsModels.size() > 0) {
                movieDetailsModel =  movieDetailsModels.get(0);
                if(movieDetailsModel.getTitle().equals(titles.get(i))){
                    continue;
                }
                System.out.println("更新数据");
            } else {
                if(titles.get(i)!=null&&!titles.get(i).equals("")){
                    movieDetailsModel = new M3U8MovieDetailsModel();
//                    testReptile.setImg(imgs.get(i));
            ...
            Spider.create(new M3U8TeleplayDetailsReptile()).addUrl(urls.get(i)).run();
        }
